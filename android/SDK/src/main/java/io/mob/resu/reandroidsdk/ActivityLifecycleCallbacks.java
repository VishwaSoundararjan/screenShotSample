...
import static io.mob.resu.reandroidsdk.AppConstants.IsVisit;
...
//
    ...
    private void sendSocketData() {
        ...
        try {
            ...
            /**
             commented the  below code while working on flutter screen short data sharing
             */
            FragmentLifecycleCallbacks fragmentLy = null;
            if (!AppConstants.isCordova) {
                // Native and and React-Native Platforms
                fragmentLy = AppLifecyclePresenter.fragmentLifecycleCallbacks;
                if (fragmentLy != null) {
...
/**
 Added the below code for getting screen shot data, and for passing it to platform
 */
//            FragmentLifecycleCallbacks fragmentLy;
//            if (AppConstants.isHyBird) {
//                if (AppConstants.hybridViewsJson != null && AppConstants.hybridViewsJson.length() > 0 && isConnected) {
//                    JSONObject jsonObject1 = new JSONObject();
//                    jsonObject1.put("mainScreenName", AppConstants.HybridScreenUrl);
//                    jsonObject1.put(manufacture, Build.MANUFACTURER);
//                    jsonObject1.put(deviceModel, Build.MODEL);
//                    jsonObject1.put(deviceType, this.isTablet(mActivity));
//                    jsonObject1.put(deviceOs, "Android");
//                    jsonObject1.put("appId", "Android" + this.isTablet(mActivity).replace(" ", "") + SharedPref.getInstance().getStringValue(mActivity, "sharedAPIKey"));
//                    jsonObject1.put("deviceId", Util.getDeviceId(mActivity));
//                    jsonObject1.put(controls, AppConstants.hybridViewsJson.toString());
//                    this.mSocket.emit(socketClientMessageData, new Object[]{jsonObject1});
//                }
//            } else {
//                fragmentLy = AppLifecyclePresenter.fragmentLifecycleCallbacks;
//                if (fragmentLy != null) {
//                    if (fragmentLy.isOldDialogFragment && FragmentLifecycleCallbacks.view != null) {
//                        this.printTree(FragmentLifecycleCallbacks.view);
//                    } else {
//                        fragmentLy.isOldDialogFragment = false;
//                        this.printTree(mActivity.getWindow().getDecorView().getRootView());
//                    }
//                } else {
//                    this.printTree(mActivity.getWindow().getDecorView().getRootView());
//                }
//
//                if (viewsJson.size() > 0 && isConnected) {
//                    JSONObject jsonObject1 = new JSONObject();
//                    jsonObject1.put("mainScreenName", mActivity.getClass().getSimpleName());
//                    jsonObject1.put(manufacture, Build.MANUFACTURER);
//                    jsonObject1.put(deviceModel, Build.MODEL);
...
//            fragmentLy = AppLifecyclePresenter.fragmentLifecycleCallbacks;
           ...
           // created this below condition to set hybrid screenshot data
            if (!AppConstants.isHyBird){
                this.ScreenBitmap = Bitmap.createBitmap(v1.getDrawingCache());
            } else {
                this.ScreenBitmap = AppConstants.hybridScreenShotBitmap;
            }

            v1.setDrawingCacheEnabled(false);
            if (fragmentLy != null) {
                (new CaptureTree()).CaptureTreeImage(fragmentLy.isOldDialogFragment, isConnected, this.abTestEnabled, mActivity, this.mSocket, handler, this.runnable, this.ScreenBitmap);
            } else {
                (new CaptureTree()).CaptureTreeImage(false, isConnected, this.abTestEnabled, mActivity, this.mSocket, handler, this.runnable, this.ScreenBitmap);
            }
