...
class CaptureTree {
    ...
    public void CaptureTreeImage(boolean isDialog, boolean isConnected, boolean abTestEnabled, Activity mActivity,
            Socket mSocket, Handler handler, Runnable runnable, Bitmap screenBitmap) {
        ...
        try {
            ...
            jsonObject1.put("appId", "Android" + isTablet(mActivity).replace(" ", "")
                    + SharedPref.getInstance().getStringValue(mActivity, AppConstants.reSharedAPIKey));
    ...
    private static String takeScreenshot(Activity mActivity) {
        ...
        try {
            ...
            screenBitmap = returnBitmap(screenBitmap,
                    pxToDp(mActivity.getWindow().getDecorView().getRootView().getWidth()),
                    pxToDp(mActivity.getWindow().getDecorView().getRootView().getHeight()));//// where mIcon_val is
                                                                                            //// bitmap to resize
