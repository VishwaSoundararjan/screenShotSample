...
class ReFlutterNavigatorObserver extends NavigatorObserver {
  ...
  void setAndTaggingScreenName() {
    ...
    if (taggingScreen.isNotEmpty) {
      ///Send screen name
      debugPrint("Received screen name :: $taggingScreen");
...
class MyNavigatorObserver extends NavigatorObserver {
  ...
  void didPush(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    print('Screen pushed: ${route.settings.name}');
  }

  @override
  void didPop(Route<dynamic> route, Route<dynamic>? previousRoute) {
    // Called when a route is popped from the navigator.
    print('Screen popped: ${route.settings.name}');
  }

// You can override other methods for additional events.
}
